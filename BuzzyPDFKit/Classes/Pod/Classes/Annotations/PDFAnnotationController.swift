...
open class PDFAnnotationController: UIViewController {
    ...
    var annType : AAnnotationType = .Highlighter
    ...
    func selected(button: PDFAnnotationBarButton) {
        ...
        if button.annotationType == PDFTextAnnotation.self
        {
            annType = .Text
        }else if button.annotationType == PDFHighlighterAnnotation.self
        {
            annType = .Highlighter
        }else if button.annotationType == PDFPenAnnotation.self
        {
            annType = .Pen
        }
    ...
    open override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        locationArr.append(point.x)
        locationArr.append(point.y)
    }
    
    open override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard let touch = touches.first else { return }
        let point = touch.location(in: pageView)
//        let parameters: Parameters = ["points":[locationArr]]
//        Network.shared.addAnnotations(params: parameters)
       
        print(annType.self)
        
        switch annType {
        case .Highlighter:
            print("This is highlight Annotation")
        case .Pen:
            print("This is Pen Annotation")
            let parameters: Parameters = ["lineDetails":["points":[locationArr]],"textDetails":["points":[0,0],"fontSize":14,"text":""]]
            Network.shared.addAnnotations(params: parameters)
        case .Text:
            print("This is Text Annotation")
           // let parameters: Parameters = ["lineDetails":["points":[locationArr]],"textDetails":["points":[0,0],"fontSize":14,"text":""]]
          //  Network.shared.addAnnotations(params: parameters)
        default:
            print("Default case")
